...
impl Default for Tokenizer {
    fn default() -> Self {
        ...
        Self::builder().build().expect("default tokenizer regex r\"\\w+\" is always valid")
    }
}
...
impl TokenizerBuilder {
    ...
    pub fn build(self) -> Result<Tokenizer> {
        let pattern = Regex::new(&self.pattern)
            .map_err(|e| crate::error::Error::TokenizationError(e.to_string()))?;
...
impl Tokenizer {
    ...
    pub fn tokenize_with_vocab(
        ...
        crate::types::Tokenized { token_ids, length }
    }
}
...
mod tests {
    ...
    fn language_german_sets_stemmer_and_stopwords() {
        ...
        assert!(!tokens.contains(&"der".to_string()), "German stopword 'der' should be removed");
    }
    ...
    fn language_french_sets_stemmer_and_stopwords() {
        ...
        assert!(!tokens.contains(&"le".to_string()), "French stopword 'le' should be removed");
    }
    ...
    fn stemmer_all_17_algorithms() {
        ...
        for algo in &algorithms {
            let tok = Tokenizer::builder()
                .stemmer(*algo)
                .build();
            assert!(tok.is_ok(), "Failed to build tokenizer with stemmer {:?}", algo);
        }
    }
    ...
    fn language_convenience_all_supported() {
        let langs = [
            ...
            "hungarian", "romanian", "turkish", "arabic",
        ];
        for lang in &langs {
    ...
    fn stemmer_italian_known_stems() {
        ...
        assert!(tokens.contains(&"gatt".to_string()), "Italian: 'gatti' should stem to 'gatt', got {:?}", tokens);
    }
    ...
    fn stemmer_portuguese_known_stems() {
        ...
        assert!(!tokens.is_empty(), "Portuguese stemmer should produce tokens");
    }
    ...
    fn stemmer_norwegian_known_stems() {
        ...
        assert!(!tokens.is_empty(), "Norwegian stemmer should produce tokens");
    }
    ...
    fn stemmer_hungarian_known_stems() {
        ...
        assert!(!tokens.is_empty(), "Hungarian stemmer should produce tokens");
    }
    ...
    fn custom_fn_completely_bypasses_builtin_pipeline() {
        ...
        let tok = Tokenizer::builder()
            ...
            .unwrap();
        let tokens = tok.tokenize("the running fox");
        assert_eq!(tokens, vec!["custom_token"], "custom_fn should override entire pipeline");
    }
    ...
    fn whitespace_only_input_returns_empty() {
        let tok = Tokenizer::default();
        let tokens = tok.tokenize("   \t\n  ");
        assert!(tokens.is_empty(), "Whitespace-only input should produce empty token list");
    }
    ...
    fn language_spanish_sets_stemmer_and_stopwords() {
        ...
        assert!(!tokens.contains(&"el".to_string()), "Spanish stopword 'el' should be removed");
    }
    ...
    fn language_italian_sets_stemmer_and_stopwords() {
        ...
        assert!(!tokens.contains(&"il".to_string()), "Italian stopword 'il' should be removed");
    }
    ...
    fn language_iso_code_works() {
        let tok = Tokenizer::builder()
            ...
            .unwrap();
        let tokens = tok.tokenize("the running fox");
        assert!(!tokens.contains(&"the".to_string()), "ISO code 'en' should set English stopwords");
    }
    ...
    fn tokenize_with_vocab_builds_vocabulary_correctly() {
        ...
        assert_eq!(result.token_ids[0], result.token_ids[2]);
    }
}
